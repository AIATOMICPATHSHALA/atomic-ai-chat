...
CREATE INDEX IF NOT EXISTS "BatchMembership_courseId_idx" ON "BatchMembership"("courseId");
...
ALTER TABLE "BatchMembership" ADD CONSTRAINT "BatchMembership_courseId_fkey" FOREIGN KEY ("courseId") REFERENCES "Course"("id") ON DELETE SET NULL ON UPDATE CASCADE;
...
INSERT INTO "BatchMembership" (
  "id", "userId", "batchId", "courseId", "enrollmentId", "status", "syncSource", "joinedAt", "updatedAt"
)
SELECT
  CONCAT('migrated-membership-', ROW_NUMBER() OVER (ORDER BY "id")),
  "userId", "batchId", "courseId", "id", "status", 'migration', "enrolledAt", CURRENT_TIMESTAMP
FROM "Enrollment"
