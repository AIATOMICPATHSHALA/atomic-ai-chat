...
function getGoogleAuthOptions() {
  ...
  if (existsSync("./service-account.json")) {
    return { keyFile: "./service-account.json" };
  }
...
export async function syncUserToStudentSheet(prisma: PrismaClient, userId: string) {
  const user = await prisma.user.findUnique({
    where: { id: userId },
    include: { profile: true, access: true },
  });
  if (!user) return;
  ...
  await syncStudentToSheet({
    ...
    atomicBatch: user.profile?.atomicBatch,
    role: user.role,
    plan: user.access?.plan ?? (user.isPro ? "PRO" : "FREE"),
    accessStatus: user.access?.status ?? (user.isSuspended ? "SUSPENDED" : "ACTIVE"),
    expiresAt: user.access?.expiresAt,
