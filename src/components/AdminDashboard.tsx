...
interface AdminUser {
  ...
  batchMemberships: Array<{ batch: { id: string; title: string } }>;
...
interface StudentPerformanceDetail {
  user: { id: string; name: string | null; email: string; atomicId: string; role: string };
  stats: {
    xp: number;
    level: number;
    currentStreak: number;
...
const updateRole = async (userId: string, role: string) => {
  ...
  const assignToBatch = async (userId: string) => {
    ...
    setError(null);
    try {
      const response = await fetch(`/api/admin/batches/${membershipBatchId}/students`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const loadPerformance = async () => {
    ...
    setError(null);
    try {
      const response = await fetch("/api/admin/student-performance", { cache: "no-store" });
  ...
  const loadPerformanceDetail = async (userId: string) => {
    ...
    setError(null);
    try {
      const response = await fetch(`/api/admin/student-performance?userId=${encodeURIComponent(userId)}`, {
        cache: "no-store",
      });
                  ...
                  {performanceStudents.map((student) => (
                    ...
                    >
                      <td className="px-3 py-3">
                        ...
                        <p className="text-xs text-slate-500">{student.atomicId}</p>
                      </td>
          ...
          {performanceDetail && !performanceDetailLoading && (
            <div className="mt-4 border-l-4 border-atomic-blue bg-slate-50 px-4 py-4 text-sm dark:bg-slate-900">
              ...
              <p className="mt-1 text-slate-500">
                {performanceDetail.user.email} - {performanceDetail.user.atomicId}
              </p>
