...
import { ArrowLeft, CalendarDays, Pencil, Plus, RefreshCw, Trash2, Youtube } from "lucide-react";
...
export function ScheduleManager() {
  ...
  const load = useCallback(async (batch: string = "") => {
    ...
    setError(null);
    try {
      const response = await fetch("/api/admin/schedule?batch=" + encodeURIComponent(batch), {
        cache: "no-store",
      });
  ...
  const handleSyncFromSheet = async () => {
    ...
    setNotice(null);
    try {
      const response = await fetch("/api/admin/schedule/sync", { method: "POST" });
  ...
  const handleDelete = async (id: string) => {
    ...
    setNotice(null);
    try {
      const response = await fetch("/api/admin/schedule?id=" + encodeURIComponent(id), {
        method: "DELETE",
      });
      const data = (await response.json()) as { error?: string };
  ...
  const handleDeleteGroup = async (ids: string[]) => {
    ...
    try {
      await Promise.all(
        ids.map((id) =>
          fetch("/api/admin/schedule?id=" + encodeURIComponent(id), { method: "DELETE" })
        )
      );
      setNotice("Class deleted.");
      await load(filterBatch);
      ...
      <main className="mx-auto flex min-h-dvh max-w-2xl items-center px-4">
        <div>
          ...
          <p className="mt-2 text-slate-500">
            You need admin or faculty access to manage the class schedule.
          </p>
